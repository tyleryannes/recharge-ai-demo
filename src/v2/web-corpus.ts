...
export const SOURCES = {
  ...
  pdpDefault: src('subscribe-by-default', 'Subscription as the default option on product pages'),
  webVitals: src('core-web-vitals-lcp', 'Core Web Vitals: Largest Contentful Paint thresholds'),
  mobileConv: src('mobile-conversion-food-beverage', 'Mobile conversion rates in food and beverage DTC'),
  helpdesk: src('support-save-offers', 'Save offers in support conversations'),
  reminder: src('pre-charge-reminders', 'Pre-charge reminders and post-charge cancellations'),
  addon: src('upcoming-order-add-ons', 'One-click add-ons in upcoming-order emails'),
  adMatch: src('ad-to-landing-message-match', 'Message match between ads and landing pages'),
  thanks: src('thank-you-page-subscription-offers', 'Subscription offers on the thank-you page'),
};
...
export const WEB_FINDINGS: WebFinding[] = [
  ...
  { key: 'b_pdp_default', agent: 'research-benchmarks', claim: 'When subscription is the default or equally prominent option on the product page, consumables brands typically see 15–25% of first orders placed as subscriptions.', evidence: 'Product-page tests across consumables brands; results depend on the discount and on showing "skip or cancel anytime".', confidence: 'medium', source: SOURCES.pdpDefault, verdict: 'supported', verdictNote: 'Benchmarks finding: 15–25%; the plan uses 13%, below the range, to stay conservative.' },
  { key: 'b_lcp', agent: 'research-benchmarks', claim: 'Google counts a Largest Contentful Paint of 2.5 seconds or less as "good"; slower pages lose shoppers, especially on mobile.', evidence: 'Core Web Vitals thresholds: good ≤ 2.5 s, poor > 4 s.', confidence: 'high', source: SOURCES.webVitals, verdict: 'supported', verdictNote: 'Benchmarks finding: 2.5 s is the "good" threshold.' },
  { key: 'b_mobile_conv', agent: 'research-benchmarks', claim: 'Food and beverage DTC stores typically convert 2–3% of mobile sessions.', evidence: 'Published conversion benchmarks by vertical and device; wide spread.', confidence: 'low', source: SOURCES.mobileConv, verdict: 'supported', verdictNote: 'Benchmarks finding: 2–3% (low confidence).' },
  { key: 'b_helpdesk', agent: 'research-benchmarks', claim: 'Support teams that offer a skip or a frequency change before processing a cancellation request keep 20–30% of those subscribers.', evidence: 'Helpdesk save-offer programmes in subscription consumables.', confidence: 'medium', source: SOURCES.helpdesk, verdict: 'supported', verdictNote: 'Benchmarks finding: 20–30%; the plan uses 20%.' },
  { key: 'b_reminder', agent: 'research-benchmarks', claim: 'Sending a reminder a few days before each subscription charge cuts cancellations and refund requests right after the charge.', evidence: 'Pre-charge reminder studies; customers who can skip in advance stop cancelling after the fact.', confidence: 'medium', source: SOURCES.reminder, verdict: 'supported', verdictNote: 'Benchmarks finding on pre-charge reminders.' },
  { key: 'b_addon', agent: 'research-benchmarks', claim: 'Upcoming-order emails with one-click add-on buttons lift add-on attach to 7–10% of subscription orders.', evidence: 'Add-on programmes using one-click links in pre-charge emails.', confidence: 'medium', source: SOURCES.addon, verdict: 'supported', verdictNote: 'Benchmarks finding: 7–10%; the plan uses 7%.' },
  { key: 'b_thanks', agent: 'research-benchmarks', claim: 'A subscription offer on the thank-you page converts 3–8% of one-time buyers on the spot.', evidence: 'Post-purchase upsell results in consumables.', confidence: 'medium', source: SOURCES.thanks, verdict: 'supported', verdictNote: 'Benchmarks finding: 3–8%.' },
  { key: 't_ad_match', agent: 'research-trends', claim: 'Landing pages that repeat the offer shoppers saw in the ad convert better than pages that show a different offer.', evidence: 'Message-match studies across paid social.', confidence: 'medium', source: SOURCES.adMatch, verdict: 'supported', verdictNote: 'Trends finding on message match.' },
